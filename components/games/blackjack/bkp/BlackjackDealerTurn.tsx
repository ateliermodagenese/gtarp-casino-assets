...
export default function BlackjackDealerTurn({
  ...
  const t = LABELS[langKey] || LABELS.br;

  // Mapear suits para path format
  ...
  const mapCard = (c: Card): Card => ({ ...c, suit: suitMap[c.suit] || c.suit });

  // Usar props reais ou fallback para mocks
  const dealerCardsRaw = dealerHandProp?.cards || DEALER_CARDS;
  const dealerCardsMapped = dealerCardsRaw.map(mapCard);
  const playerCardsRaw = playerHandsProp?.[0]?.cards || PLAYER_CARDS;
  const playerCardsMapped = playerCardsRaw.map(mapCard);
  const finalDealerTotal = dealerHandProp?.total ?? 26;
  const finalPlayerTotal = playerHandsProp?.[0]?.total ?? 21;
  const isDealerBust = dealerHandProp?.isBust ?? (finalDealerTotal > 21);
