...
export default function BlackjackSplit({
  onAction,
  lang = "br",
  playerHands: playerHandsProp,
  activeHandIndex: activeHandIndexProp,
  dealerHand: dealerHandProp,
  dealerVisibleTotal,
  ...
  const mapCard = (c: Card): Card => ({ ...c, suit: (suitMap[c.suit] || c.suit) as Suit });

  // Usar props reais ou fallback mocks
  const activeIdx = activeHandIndexProp ?? MOCK_ACTIVE_INDEX;
  const dealerCards = dealerHandProp ? dealerHandProp.cards.map(mapCard) : MOCK_DEALER.cards;
  const dealerTotal = dealerVisibleTotal ?? dealerHandProp?.total ?? MOCK_DEALER.total;
  const playerHands: HandData[] = playerHandsProp
    ? playerHandsProp.map(h => ({ ...h, cards: h.cards.map(mapCard) }))
    : MOCK_HANDS;

  const handleAction = (action: Action) => {
    const actionUpper = action.toUpperCase() as any;
    onAction?.(actionUpper);
  };

  const isActionEnabled = (action: Action): boolean => {
    if (availableActionsProp) {
      return availableActionsProp.includes(action.toUpperCase());
    }
    switch (action) {
      case "hit":
      case "stand":
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {t(TEXTS.playingHand, langKey)} {activeIdx + 1} {t(TEXTS.of, langKey)} {playerHands.length}
        </span>
